"""
app.py — FastAPI web server for CustomerSupportEnv HF Space

Exposes the OpenEnv interface over HTTP so the automated validator can:
  - Ping GET  /          → 200 health check
  - Call POST /reset     → env.reset()
  - Call POST /step      → env.step(action)
  - Call GET  /state     → env.state()
  - Call GET  /tasks     → list all tasks

Also runs inference.py in a background subprocess on startup.
"""
...
# Per-task environment instances (keyed by task_id)
...
# ── Startup: run inference.py in background ───────────────────────────────────
